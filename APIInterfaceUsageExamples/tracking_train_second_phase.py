...
if __name__ == '__main__':
    ...
    out_dict = VQPPF_module(template_patch, search_patch_list, time_variant_object_state, train_first_phase=False)
    z_q_list = out_dict['z_q']
    z_pre, lstm_loss = LSTM(z_q_list[1], z_q_list[2], z_q_list[3], need_pre=True)
    z_q_list[-1] = z_pre
    object_tv, recon_loss = VQPPF_module.decode(z_q_list, time_variant_object_state)

    vae_loss = recon_loss + out_dict['total_loss']
